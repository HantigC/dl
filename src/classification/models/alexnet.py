...
class AlexNet(nn.Module):
    ...
    def __init__(self, classes_num):
        ...
        self.conv21 = nn.Conv2d(in_channels=48, out_channels=128, kernel_size=5, padding=2)
        self.max_pool21 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.conv22 = nn.Conv2d(in_channels=48, out_channels=128, kernel_size=5, padding=2)
        self.max_pool22 = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.conv31 = nn.Conv2d(in_channels=256, out_channels=192, kernel_size=3, padding=1)
        self.max_pool31 = nn.MaxPool2d(kernel_size=3, stride=2)
        self.conv32 = nn.Conv2d(in_channels=256, out_channels=192, kernel_size=3, padding=1)
        self.max_pool32 = nn.MaxPool2d(kernel_size=3, stride=2)

        self.conv41 = nn.Conv2d(in_channels=192, out_channels=192, kernel_size=3, padding=1)
        self.conv42 = nn.Conv2d(in_channels=192, out_channels=192, kernel_size=3, padding=1)

        self.conv51 = nn.Conv2d(in_channels=192, out_channels=128, kernel_size=3, padding=1)
        self.conv52 = nn.Conv2d(in_channels=192, out_channels=128, kernel_size=3, padding=1)

        self.flatten = nn.Flatten()

        self.ffn11 = LinearDefered(out_features=1024)
        self.ffn11_d = nn.Dropout()
        self.ffn12 = LinearDefered(out_features=1024)
    ...
    def forward(self, x):
        ...
        st_part = self.conv51(F.relu(self.conv41(self.max_pool31(F.relu(self.conv31(x))))))
        nd_part = self.conv52(F.relu(self.conv42(self.max_pool32(F.relu(self.conv32(x))))))
        x = torch.cat([st_part, nd_part], dim=1)
        x = self.flatten(x)
